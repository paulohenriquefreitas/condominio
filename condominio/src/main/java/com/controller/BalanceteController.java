...
public class BalanceteController {
		...
		public String list(Model model , @RequestParam ("dataRange") String dataRange){
			...
			model.addAttribute("totalCondominioAnterior",recebimentoDao.findRecebimento("01-01-2015", datas[0].trim(), "condominio",1));
			model.addAttribute("totalFundo",recebimentoDao.findRecebimento(datas[0].trim(), datas[1].trim(), "fundo de reserva",null));
			model.addAttribute("totalFundoAnterior",recebimentoDao.findRecebimento("01-01-2015", datas[0].trim(), "fundo de reserva",1));
			model.addAttribute("totalCota",recebimentoDao.findRecebimento(datas[0].trim(), datas[1].trim(), "cota extra",null));	
			model.addAttribute("totalCotaAnterior",recebimentoDao.findRecebimento("01-01-2015", datas[0].trim(), "cota extra",1));	
			...
			model.addAttribute("totalPagamentoOrdinarioAnterior",pagamentoDao.findPagamento("01-01-2015", datas[0].trim(), "condominio",1));
			model.addAttribute("totalPagamentoFundoAnterior",pagamentoDao.findPagamento("01-01-2015", datas[0].trim(), "fundo de reserva",1));
			model.addAttribute("totalPagamentoExtraAnterior",pagamentoDao.findPagamento("01-01-2015", datas[0].trim(), "cota extra",1));
